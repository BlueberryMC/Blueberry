...
public class ModManager {
    ...
    public void loadPacks(@NotNull Consumer<Pack> consumer, @NotNull Pack.PackConstructor packConstructor) {
        for (BlueberryMod mod : Blueberry.getModLoader().getLoadedMods()) {
            try {
                Pack pack = Pack.create(mod.getDescription().getModId(), false, () -> mod.getResourceManager().getPackResources(), packConstructor, Pack.Position.BOTTOM, PackSource.BUILT_IN);
